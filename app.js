...
const chartSets={
  '24h':{real:[28,30,29,34,37,41,48,52,59,64,72,78,86],expected:[27,29,30,32,35,38,42,46,50,54,59,63,68],labels:['00h','06h','12h','18h','Agora'],waste:'11,3 kWh',period:'nas últimas 24 horas'},
  '7d':{real:[58,64,69,75,82,91,104],expected:[55,58,62,66,71,76,81],labels:['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'],waste:'68,7 kWh',period:'nos últimos 7 dias'}
};
function renderChart(period='24h'){
  const data=chartSets[period],svg=$('energySvg'),w=720,top=10,bottom=218,h=bottom-top;
  const max=Math.ceil(Math.max(...data.real,...data.expected)/20)*20;
  const xy=(value,index,list)=>[index*(w/(list.length-1)),bottom-(value/max)*h];
  const real=data.real.map((v,i)=>xy(v,i,data.real)),expected=data.expected.map((v,i)=>xy(v,i,data.expected));
  const line=arr=>arr.map(p=>p.join(',')).join(' '),area=`M${real[0][0]},${bottom} L${real.map(p=>p.join(',')).join(' L')} L${real.at(-1)[0]},${bottom} Z`;
  const grid=[0,.25,.5,.75,1].map(f=>`<path d="M0 ${top+h*f}H${w}"/>`).join('');
  const hitWidth=w/data.real.length;
  svg.innerHTML=`<defs><linearGradient id="energyArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ff9d57" stop-opacity=".35"/><stop offset="1" stop-color="#ff9d57" stop-opacity="0"/></linearGradient></defs><g class="chart-grid">${grid}</g><path class="energy-area" d="${area}"/><polyline class="expected-line" points="${line(expected)}"/><polyline class="real-line" points="${line(real)}"/>${real.map(p=>`<circle class="data-point" cx="${p[0]}" cy="${p[1]}" r="4"/>`).join('')}${real.map((p,i)=>`<rect class="chart-hover" data-index="${i}" x="${Math.max(0,p[0]-hitWidth/2)}" y="0" width="${hitWidth}" height="230"/>`).join('')}`;
  $('chartScale').innerHTML=[max,max*.75,max*.5,max*.25,0].map(v=>`<span>${Math.round(v)}</span>`).join('');
  $('chartLabels').innerHTML=data.labels.map(v=>`<span>${v}</span>`).join('');
  $('chartInsight').innerHTML=`Foram identificados <strong>${data.waste} de desperdício excedente</strong> neste circuito ${data.period}.`;
